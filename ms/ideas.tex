...
\begin{itemize}
\item outcomes of long-term evolutionary adaptation. More than just intrinsic interest, as path towards understanding how vegetation may change as result of environmental change.
\end{itemize}
...
Requires demographic trade-off between growth in high-light and shade-tolerance. Possible via trade-offs in either LMA or wood density,


\begin{enumerate}
\item Change in productivity
  \begin{itemize}
  \item  high LMA as adaptation to both log light and low water
  \end{itemize}
\item Change the relationship LMA and turnover
  \begin{itemize}
  \item slope or elevation
  \end{itemize}
\item Introduce additional trait. 3 possible outcomes
  \begin{itemize}
    \item compensates for lower water, leaf N per area
      \begin{itemize}
        \item make LMA shift less than would have been otherwise
        \item instead differentiate species in N (with flow on effect to shade tolerance) and also in LL
      \end{itemize}
    \item ameliorates risk of mortality
      \begin{itemize}
        \item(do we know which trait changes embolism risk)
        \item psi50: don't know which traits
        \item vessel size:
        \begin{itemize}
          \item assume has some carbon cost, otherwise not beneficial
          \item diverts carbon away from growth, so amplifies change in LMA
        \end{itemize}
      \end{itemize}
      \item height at maturation - will decrease difference in LMA
  \end{itemize}
\end{enumerate}
