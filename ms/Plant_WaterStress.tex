...
\item I need to change or add the annual photosynthesis in the C code to allow to use also a non-rectangular hyperbola in  ff16 strategy. 

\item I need to choose a parameterisation for the FvC model (this seems to be quite variable in fact). I propose to use either TROLL or Sterck.

\item I need to decide the relationship between $V_{cmax}$ and $N_{area}$ \footnote{We need to discuss the idea of splitting $N_{area}$ in structural and Rubisco}. It seems simple to start with \citet{Sakschewski-2015}.
\end{itemize}
