...
\item In plantecophys (ADD REF)  $\alpha = 0.24$ and $\theta = 0.85$
\item In Medlyn et al. 2002 $\alpha = 0.3$ and $\theta = 0.9$
...
\item In Sterck et al. 2011 $\theta = 0.5$ and $\alpha = 0.25$ (not exactly the same alpha ...)
\item  In Valladares et al. 1997 tropicla shrub $\theta$ between 0.51 and 0.8 and $\alpha$ (in mol CO2) between 0.048 and 0.066
\item In TROLL Marechau \& Chave Ecol. Monogr. in press $\alpha = 0.075*4 = 0.3$ and $\theta = 0.7$ based on von Caemmerer 2000

\item In von  Caemmerer 2000 the non rectangular hyperbola light response curve have an initial slope $\alpha_1 \times \PHI_{PSII} \times \beta$ with $\alpha_1$ the leaf absorptance, $\PHI_{PSII}$ the maximum quantum yield of the photosystem II and $\beta$ the fraction of absorbed light that reach PSII.
\end{itemize}
...
- in Walker, A. P. et al. 2014. The relationship of leaf photosynthetic traits - Vcmax and Jmax - to leaf nitrogen, leaf phosphorus, and specific leaf area: a meta-analysis and modeling study. - Ecology and Evolution 4: 3218–3235.
It is a power function exp(1.01)*V^0.89
- in Try data from Kattge et al. 2011 exp(1.669)*V^0.75,
- in Wullschleger 1993 exp(1.425)*V^0.837

ADD Figure Jmax vs Vcmax
...
#Vcmax vs Narea
Photosynthetic rate should scale positively with leaf Nitrogen (N) because of teh large amount of N invested in RuBisCO. Several relationship have been proposed to link Vcmax and leaf N.
- Sakschewski et al. 2014 Vcmax = 31.62*Narea^0.801 but UNIT CONVERSION of g per m2 to kg per m2 *1000^0.801
- ln(Vcmax) = 3.946 + 0.921 * log(N) + 0.121 * log(P) + 0.281*log(N)*log(P), Walker et al. 2014. To get a function of only P take the mean leaf P (in TRY mean leaf P per area is 0.104 g m-2, in Walker et al. 2014 data this is 0.135)
- Kattge et al. 2009. Vcmax = a + b * N with different value per Biomes estimated either with Vcmax onl or also constrained by Amax.
- Walker et al. 2017 explore several alternative model for Vcmax:a modificytion of Walker et al. is
Vcmax = exp(3.712)*N^0.650

- Domingues et al. 2011
Vcmax = 10^1.57*N^0.55 # Appendix C Table A2

ADD FIGURE Vcmax Narea

Other traits such as leaf P and SLA could also influence Vcmax (Domingues et al. 2010, Walker et al. 2014). SLA have little effect on the link between Vcmax and leaf N.  Leaf P have little direct effect on Vcmax but an important interaction with leaf N: at low leaf P the slope of Vcmax vs leaf N is shallow but at high leaf P thisslope is steep (see Walker et al. 2014).
...
\item using stomatal model that is linked to the soil water potential (as in \citet{Spery-2016} or \citet{Sterck-2011})
