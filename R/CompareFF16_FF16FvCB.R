...
data_patch_FF16 <- function(prod = 0, cancel_F = FALSE, v_narea = c(1e-3, 3e-3, 5e-3)){
...
p0 <- trait_gradients_base_parameters(site_prod=prod)
p0$control <- ctrl
...
##         ylim = c(1e-27, max(density1, density2, density3, na.rm = TRUE)),
...
compare_strategy_patch<- function(v_narea = c(1e-3, 3e-3, 5e-3)){
require(plant)
...
#
p02 <- trait_gradients_base_parameters(site_prod=-0.3)
p02$control <- ctrl
p02$disturbance_mean_interval <- 30.0

## First, with three species:
p2 <- expand_parameters(trait_matrix(v_narea, "narea"), p02, FALSE)

p_eq2 <- equilibrium_seed_rain(p2)

## Then collect the patch-level dynamics:
data2 <- run_scm_collect(p_eq2)

tt <- data2$time
h1 <- data2$species[[1]]["height", , ]
h2 <- data2$species[[2]]["height", , ]
h3 <- data2$species[[3]]["height", , ]

h1 <- data2$species[[1]]["seeds_survival_weighted", , ]
h2 <- data2$species[[2]]["seeds_survival_weighted", , ]
h3 <- data2$species[[3]]["seeds_survival_weighted", , ]
...
d3 <- data2$species[[3]]["log_density", , ]
...
# size structure
height1 <- t(data2$species[[1]]["height", , ])
log_density1 <- t(data2$species[[1]]["log_density", , ])
density1 <- exp(log_density1)
height2 <- t(data2$species[[2]]["height", , ])
log_density2 <- t(data2$species[[2]]["log_density", , ])
density2 <- exp(log_density2)
height3 <- t(data2$species[[3]]["height", , ])
log_density3 <- t(data2$species[[3]]["log_density", , ])
density3 <- exp(log_density3)
matplot(height1, density1, type="l", lty=1,
        col= make_transparent( cols[1], 0.25),
        ylim = c(1e-27, max(density1, density2, density3, na.rm = TRUE)),
...
# size structure
height1 <- t(dataF$species[[1]]["height", , ])
log_density1 <- t(dataF$species[[1]]["log_density", , ])
density1 <- exp(log_density1)
height2 <- t(dataF$species[[2]]["height", , ])
log_density2 <- t(dataF$species[[2]]["log_density", , ])
density2 <- exp(log_density2)
height3 <- t(dataF$species[[3]]["height", , ])
log_density3 <- t(dataF$species[[3]]["log_density", , ])
density3 <- exp(log_density3)
matplot(height1, density1, type="l", lty=1,
        col= make_transparent( cols[1], 0.25),
        ylim = c(1e-27, max(density1, density2, density3, na.rm = TRUE)),
...
## high
pF02 <- FF16FvCB_Parameters(hyperpar = make_FF16FvCB_hyperpar(vpd = 3))
pF02$control <-  ctrl
pF02$disturbance_mean_interval <- 30.0

## First, with three species:
pF2 <- expand_parameters(trait_matrix(v_narea, "narea"), pF02, FALSE)

pF_eq2 <- equilibrium_seed_rain(pF2)

## Then collect the patch-level dynamics:
dataF2 <- run_scm_collect(pF_eq2)

tt <- dataF2$time
h1 <- dataF2$species[[1]]["height", , ]
h2 <- dataF2$species[[2]]["height", , ]
h3 <- dataF2$species[[3]]["height", , ]

cols <- c("#e41a1c", "#377eb8", "#4daf4a")

## Relativise the log densities onto (-4, max)
d1 <- dataF2$species[[1]]["log_density", , ]
d2 <- dataF2$species[[2]]["log_density", , ]
d3 <- dataF2$species[[3]]["log_density", , ]
...
# size structure
height1 <- t(dataF2$species[[1]]["height", , ])
log_density1 <- t(dataF2$species[[1]]["log_density", , ])
density1 <- exp(log_density1)
height2 <- t(dataF2$species[[2]]["height", , ])
log_density2 <- t(dataF2$species[[2]]["log_density", , ])
density2 <- exp(log_density2)
height3 <- t(dataF2$species[[3]]["height", , ])
log_density3 <- t(dataF2$species[[3]]["log_density", , ])
density3 <- exp(log_density3)
matplot(height1, density1, type="l", lty=1,
        col= make_transparent( cols[1], 0.25),
        ylim = c(1e-27, max(density1, density2, density3, na.rm = TRUE)),
...
compare_strategy_patch_Fecundity<- function(v_narea = c(1e-3, 3e-3, 5e-3)){
require(plant)
ctrl <- equilibrium_verbose(fast_control())
ctrl$equilibrium_solver_name <- "iteration"
#ctrl$equilibrium_verbose <-  TRUE
...
## First, with three species:
p2 <- expand_parameters(trait_matrix(v_narea, "narea"), p0, FALSE)
