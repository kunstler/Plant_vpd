...
get_climate_niche_sp <- function(sp, wc) {
  ...
  dat <- tryCatch(occ_search(taxonKey = key, return = "data", fields = c("name",
    "decimalLatitude", "decimalLongitude", "issues")), error = function(err) NA)
  if (is.data.frame(dat)) {
    dat <- dat %>% occ_issues(-cdout, -cdrepf, -cucdmis, -preswcd, -zerocd)
    dat <- dat[!is.na(dat$decimalLatitude) & dat$decimalLatitude != 0 & !is.na(dat$decimalLongitude) &
      dat$decimalLongitude != 0, ]
    ...
    if (nrow(dat) > 0) {
      ...
      if (nrow(df_clim) > 0) {
        ...
        res <- data.frame(Binomial = sp, do.call("cbind", df_wc), nobs = nrow(dat),
          stringsAsFactors = FALSE)
        names(res) <- c("Binomial", "MAT", "MAP", "TMAX", "PDRY", "nobs")
        print(paste("species ", sp, " done"))
      } else {
        res <- data.frame(Binomial = sp, MAT = NA, MAP = NA, TMAX = NA,
          PDRY = NA, nobs = 0, stringsAsFactors = FALSE)
        print(paste("species ", sp, " error in wc no data with climate"))
      }
    } else {
      res <- data.frame(Binomial = sp, MAT = NA, MAP = NA, TMAX = NA, PDRY = NA,
...
plot_sma <- function(df, var_x, var_y, log = "xy") {
  ...
  par(mar = c(4.6, 4.6, 0.5, 0.5))
  plot(NA, type = "n", log = log, xlim = range(df[[var_x]], na.rm = TRUE), ylim = range(df[[var_y]],
    na.rm = TRUE), xlab = "", ylab = "", las = 1)
